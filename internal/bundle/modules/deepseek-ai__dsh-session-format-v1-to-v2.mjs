// .harness/packages/session/session-format-v1-to-v2/lib/index.js
...
var retained = Object.fromEntries(Object.entries(RELEASED_V0_EVENT_DISPOSITIONS).filter(([type]) => type !== "assistant/chunk" && type !== "assistant/message" && type !== "session-log-deepseek/delivery-accepted" && type !== "session/end-seed"));
var RELEASED_V2_EVENT_DISPOSITIONS = Object.freeze({
  ...retained,
...
var RELEASED_V2_EVENT_TYPES = Object.freeze(Object.keys(RELEASED_V2_EVENT_DISPOSITIONS).sort((left, right) => left.localeCompare(right, "en")));
var HEADER_REQUIRED$1 = [
  "version",
  "id",
  "createdAt",
  "isSeeded",
...
var HEADER_OPTIONAL$1 = [
  "cwd",
  "parentSession",
  "origin",
  "agentPreset"
];
var EVENT_REQUIRED$1 = [
  "type",
  "seq",
  "time",
  "data"
...
var SURFACE_OPTIONAL = [
  "ignorable",
  "sourceEventSeqs",
  "surfaceOp"
];
var LOG_OPTIONAL = ["ignorable"];
var RELEASED_V2_RELATIONSHIP_EXTENSIONS = {
  stepEvents: /* @__PURE__ */ new Set(["assistant/attempt"]),
  preservedSourceTitleRequestText: true
};
function assertReleasedV2Header(header) {
  const record2 = releasedV2Record(header, "format v2 header");
  assertReleasedV2Keys(record2, HEADER_REQUIRED$1, HEADER_OPTIONAL$1, "format v2 header");
  if (record2["version"] !== 2) throw new SessionFormatError("expected format v2 header");
  if (typeof record2["id"] !== "string") throw new SessionFormatError("format v2 header id must be a string");
  sessionFormatCount(record2["createdAt"], "format v2 header createdAt");
  sessionFormatCount(record2["delegationDepth"], "format v2 header delegationDepth");
  if (typeof record2["isSeeded"] !== "boolean") throw new SessionFormatError("format v2 header isSeeded must be boolean");
  if (record2["cwd"] !== void 0 && (typeof record2["cwd"] !== "string" || !isAbsolute(record2["cwd"]))) throw new SessionFormatError("format v2 header cwd must be absolute");
  for (const key of ["parentSession", "agentPreset"]) if (record2[key] !== void 0 && typeof record2[key] !== "string") throw new SessionFormatError(`format v2 header ${key} must be a string`);
  if (record2["origin"] !== void 0 && record2["origin"] !== "subagent") throw new SessionFormatError('format v2 header origin must be "subagent"');
}
function validateReleasedV2Artifact(artifact, mode, knownEventTypes, relationshipHeaderVersion = artifact.header.version) {
  ...
  for (const [index, event] of artifact.events.entries()) {
    const record2 = releasedV2Record(event, `format v2 event ${index}`);
    ...
    const ignorableUnknown = disposition === void 0 && record2["ignorable"] === true;
    ...
    assertReleasedV2Keys(record2, EVENT_REQUIRED$1, mode === "physical" || disposition === void 0 ? SURFACE_OPTIONAL : surface ? SURFACE_OPTIONAL : LOG_OPTIONAL, `format v2 event ${index}`);
    if (record2["seq"] !== index) throw new SessionFormatError(`format v2 event ${index} is not dense`);
    sessionFormatSafeInteger(record2["time"], `format v2 event ${index} time`);
    if (record2["ignorable"] !== void 0 && record2["ignorable"] !== true) throw new SessionFormatError(`format v2 event ${index} ignorable must be true when present`);
...
var EVENT_KEYS = /* @__PURE__ */ new Set([...EVENT_REQUIRED, ...EVENT_OPTIONAL]);
var releasedV2SessionFormatCodec = Object.freeze({
  version: 2,
  decodeHeader(value) {
...
function decodePhysicalHeader(value) {
  const record2 = jsonRecord(snapshotSessionFormatJson(value, "released v2 physical header"), "released v2 physical header");
  exactKeys(record2, HEADER_REQUIRED, HEADER_OPTIONAL, "released v2 physical header");
  if (record2["type"] !== "session" || record2["version"] !== 2) throw new SessionFormatError("expected released v2 physical Session header");
  if (typeof record2["id"] !== "string") throw new SessionFormatError("released v2 header id must be a string");
  const createdAt = sessionFormatCount(record2["createdAt"], "released v2 header createdAt");
  const delegationDepth = sessionFormatCount(record2["delegationDepth"], "released v2 header delegationDepth");
  if (typeof record2["isSeeded"] !== "boolean") throw new SessionFormatError("released v2 header isSeeded must be boolean");
  for (const key of [
    "cwd",
    "parentSession",
    "agentPreset"
  ]) if (record2[key] !== void 0 && typeof record2[key] !== "string") throw new SessionFormatError(`released v2 header ${key} must be a string`);
  if (record2["origin"] !== void 0 && record2["origin"] !== "subagent") throw new SessionFormatError('released v2 header origin must be "subagent"');
  const header = snapshotSessionFormatJson({
    version: 2,
    id: record2["id"],
    createdAt,
    ...record2["cwd"] === void 0 ? {} : { cwd: record2["cwd"] },
    ...record2["parentSession"] === void 0 ? {} : { parentSession: record2["parentSession"] },
    isSeeded: record2["isSeeded"],
    ...record2["origin"] === void 0 ? {} : { origin: record2["origin"] },
    delegationDepth,
    ...record2["agentPreset"] === void 0 ? {} : { agentPreset: record2["agentPreset"] }
...
function decodeEvent(value, rowIndex) {
  const record2 = jsonRecord(value, `released v2 row ${rowIndex}`);
  const missing = EVENT_REQUIRED.find((key) => !Object.hasOwn(record2, key));
  if (missing !== void 0) throw new SessionFormatError(`released v2 row ${rowIndex} lacks required field ${missing}`);
  const unexpected = Object.keys(record2).find((key) => !EVENT_KEYS.has(key));
  if (unexpected !== void 0) throw new SessionFormatError(`released v2 row ${rowIndex} has unexpected field ${unexpected}`);
  if (typeof record2["type"] !== "string") throw new SessionFormatError(`released v2 row ${rowIndex} type must be a string`);
  sessionFormatSafeInteger(record2["time"], `released v2 row ${rowIndex} time`);
  if (record2["ignorable"] !== void 0 && record2["ignorable"] !== true) throw new SessionFormatError(`released v2 row ${rowIndex} ignorable must be true when present`);
  if (record2["sourceEventSeqs"] === void 0) return record2;
  const seq = sessionFormatCount(record2["seq"], `released v2 row ${rowIndex} seq`);
  return {
    ...record2,
    sourceEventSeqs: decodeSeqRanges(record2["sourceEventSeqs"], seq)
  };
}
...
function exactKeys(record2, required, optional, label) {
  const allowed = /* @__PURE__ */ new Set([...required, ...optional]);
  const missing = required.find((key) => !Object.hasOwn(record2, key));
  if (missing !== void 0) throw new SessionFormatError(`${label} lacks ${missing}`);
  const unexpected = Object.keys(record2).find((key) => !allowed.has(key));
  if (unexpected !== void 0) throw new SessionFormatError(`${label} has unexpected field ${unexpected}`);
}
var CHUNK_EVENT_REQUIRED = [
  "type",
  "seq",
  "time",
  "data"
];
var CHUNK_EVENT_OPTIONAL = [
  "ignorable",
  "sourceEventSeqs",
  "surfaceOp"
];
var CHUNK_EVENT_KEYS = /* @__PURE__ */ new Set([...CHUNK_EVENT_REQUIRED, ...CHUNK_EVENT_OPTIONAL]);
var sessionFormatV1ToV2 = defineSessionFormatMigration({
...
function recordLastTime(record2) {
  if (record2.type === "chunk") return record2.time;
  return record2.dt.reduce((time, gap) => time + gap, record2.time0);
}
function mutableRecord(record2) {
  if (record2.type === "chunk") return record2;
  if (record2.type === "tool-call-chunks") return {
    ...record2,
    dt: [...record2.dt],
    args: [...record2.args]
  };
...
function flushAccumulator(group) {
  ...
  for (const record2 of accumulator.snapshot()) appendStreamRecord(group, record2, recordLastTime(record2), false);
  delete group.accumulator;
}
function streamOf(group) {
  flushAccumulator(group);
  return group.stream.map(({ record: record2 }) => record2);
}
...
function remapPayloadReferences(event, mapping) {
  ...
  switch (event.type) {
    case "command/done":
      return data["sourceEventSeq"] === void 0 ? data : {
        ...data,
