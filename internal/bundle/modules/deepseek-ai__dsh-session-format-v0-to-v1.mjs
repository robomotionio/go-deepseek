...
var disposition = defineReleasedPayloadDisposition;
var RELEASED_V0_EVENT_DISPOSITIONS = Object.freeze({
...
var RELEASED_V0_EVENT_TYPES = Object.freeze(Object.keys(RELEASED_V0_EVENT_DISPOSITIONS).sort((left, right) => left.localeCompare(right, "en")));
...
function messageValue(value, label, version, expected) {
  ...
  if (expected === "tool") {
    ...
    const source2 = releasedV0Record(message["source"], `${label} source`);
    if (block?.["type"] !== "tool-result" || block["toolCallId"] !== source2["callId"]) throw new SessionFormatError(`${label} must contain exactly one tool-result block`);
  }
}
...
var LEGACY_SOURCE_TYPES = /* @__PURE__ */ new Set([
  "steering/message",
...
function assertReleasedArtifactCoordinates(artifact, allowLegacySteering, knownEventTypes, vocabularyNeutral = false, frozenEnvelope = false) {
  ...
  for (let index = 0; index < artifact.events.length; index += 1) {
    ...
    const disposition2 = RELEASED_V0_EVENT_DISPOSITIONS[type];
    ...
    const surface = disposition2 !== void 0 ? SURFACE_EVENT_TYPES.has(type) : type === "steering/message";
...
function assertReleasedEventPayload(event, version) {
  const disposition2 = RELEASED_V0_EVENT_DISPOSITIONS[event.type];
  if (disposition2 === void 0) throw new SessionFormatUnsupportedMigrationError(`format v0 contains unknown historical event type ${JSON.stringify(event.type)} at seq ${event.seq}; migration refuses unknown historical events even when ignorable`);
  const data = releasedV0Record(event.data, `${event.type} ${event.seq} data`);
  ...
  const versionOptional = version === 1 && event.type === "session-log-deepseek/delivery-accepted" ? [...disposition2.optional, "sessionFormatVersion"] : disposition2.optional;
  assertReleasedV0Keys(data, disposition2.required, versionOptional, `${event.type} ${event.seq} data`);
  for (const key of disposition2.opaque) if (Object.hasOwn(data, key) && !isJsonValue(data[key])) throw new SessionFormatError(`${event.type} ${event.seq} opaque ${key} is not lossless JSON`);
  assertReleasedPayloadSemantics(event, version);
}
var PHYSICAL_HEADER_REQUIRED = [
  "type",
  "version",
  "id",
  "createdAt",
...
var PACKED_TAGS = /* @__PURE__ */ new Set([
  "text-chunks",
  "reasoning-chunks",
  "tool-call-chunks"
]);
...
var releasedV0SessionFormatCodec = createReleasedCodec(0);
var releasedV1SessionFormatCodec = createReleasedCodec(1);
...
var LEGACY_ASSISTANT_SOURCE_KEY = ["pro", "venance"].join("");
var sessionFormatV0ToV1 = defineSessionFormatMigration({
...
function normalizeLegacyCompaction(event, sessionId, state) {
  ...
  if (event.type === "compaction/start") {
    const data2 = releasedV0Record(event.data, `compaction/start ${event.seq} data`);
    const existing = data2["compactionId"];
    if (typeof existing === "string" && existing.length > 0) {
      state.compactionId = existing;
      return event;
    }
    if (Object.hasOwn(data2, "compactionId")) return event;
    ...
    switch (event.type) {
      ...
      case "llm/retry-started": {
        const scheduled = retries.find((candidate) => {
          const prior2 = candidate.data;
          return prior2["retryId"] === data["retryId"] && prior2["retry"] === data["retry"];
        });
